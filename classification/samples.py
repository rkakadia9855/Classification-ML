...
def loadDataFile(filename, n,width,height):
  ...
  fin.reverse()
  items = []
  for i in range(n): # Goes through all images
    data = []
    for j in range(height): # Adds an individual image Datum object to items
...
def readlines(filename):
  ...
  if(os.path.exists(filename)): 
    return [l[:-1] for l in open(filename).readlines()] # This basically returns a list that has all the lines read. [:-1] removes the last character which could be the newline character
  else: 
